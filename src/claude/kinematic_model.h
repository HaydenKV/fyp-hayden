...
public:
  ...
  struct ModelParams {
    ...
    ModelParams() : L(0.258) {  // QCar wheelbase
      q.setOnes();
    }
  };
   ...
   */
  StateVec predict(const StateVec& x_in, double t, double dt);
   ...
   */
  StateMat getProcessJacobian(const StateVec& x, double dt);
   ...
   */
  InputMat getInputJacobian(const StateVec& x, double dt);
   ...
   */
  StateMat getProcessNoise(double dt);
   ...
   */
  Eigen::Matrix<double, INPUT_SIZE, INPUT_SIZE> getInputNoise(double dt);
...
private:
   ...
   */
  StateVec stateDot(const StateVec& x, const InputVec& u);
};
